...
Identify semantically distinct topics. A "topic" is a cohesive area of knowledge that an agent would want to look up as a unit. Examples of good topic boundaries for imbh-c:

- A specific mechanism (e.g., "Arrow C Data Interface batch handoff", "the ArrowArrayStream ownership/release model", "the panic guard and error-code mapping")
- A cross-cutting concern (e.g., "the single-arrow rule and cbindgen header generation", "typed query builders backed by proto TryFrom")
